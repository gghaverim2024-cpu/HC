...
router.post('/login', rateLimit({ max: 10, windowMs: 60000 }), (req, res) => {
...
router.post('/forgot-password', rateLimit({ max: 5, windowMs: 60000 }), (req, res) => {
  const { email } = req.body || {};
  const user = get('SELECT * FROM users WHERE email = ?', [email]);
  // Always respond the same way whether or not the email exists, to avoid leaking account existence.
  if (!user) return res.json({ ok: true });
  const token = nanoid(24);
  run('UPDATE users SET reset_token = ? WHERE id = ?', [token, user.id]);
...
router.post('/reset-password', rateLimit({ max: 10, windowMs: 60000 }), (req, res) => {
  ...
  const passwordHash = bcrypt.hashSync(newPassword, 10);
  run('UPDATE users SET password_hash = ?, reset_token = NULL WHERE id = ?', [passwordHash, user.id]);
  res.json({ ok: true });
});
